...
class CategorialDistribution(Distribution):
    ...
    def prob_distribution(self, action_logits: torch.Tensor) -> Self:
...
class MultiCategoricalDistribution(Distribution):
    ...
    def prob_distribution(self, action_logits: torch.Tensor) -> Self:
        self.distribution = [
            Categorical(logits=split_logit)
            for split_logit in torch.split(action_logits, list(self.action_dims), dim=1)
        ]
        return self
